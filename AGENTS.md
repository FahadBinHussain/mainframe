...
- secrets (api keys, tokens, oauth json) live in the profile dir as `api-key.txt` / `token.txt` / `oauth.json`, written by the helper's `login`/`token-add` flow. `backup-secrets.ps1` snapshots the whole accounts dir so profiles travel with every backup/restore.
